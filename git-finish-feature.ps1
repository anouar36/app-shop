...
Write-Host "Push de develop..." -ForegroundColor Cyan
git push origin develop

# Demander si on veut supprimer la branche feature
$deleteFeature = Read-Host "Supprimer la branche feature/$FeatureName ? (y/N)"
if ($deleteFeature -eq "y" -or $deleteFeature -eq "Y") {
    git branch -d "feature/$FeatureName"
    git push origin --delete "feature/$FeatureName"
    Write-Host "Branche feature supprimee" -ForegroundColor Red
}

Write-Host "Feature $FeatureName integree avec succes dans develop!" -ForegroundColor Green
